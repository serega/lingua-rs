...
fn get_file_content<'a>(file_name: &'a str, language: &'a Language) -> Vec<String> {
...
fn main() {
    ...
    for (_, language) in Language::iter().enumerate() {
        ...
        all_examples.extend_from_slice(single_words.as_slice());
        all_examples.extend_from_slice(word_pairs.as_slice());
        all_examples.extend_from_slice(sentences.as_slice());

        counts.insert(language.clone(), 0);
    }

    let mut rng = thread_rng();
    all_examples.shuffle(&mut rng);

    let mut max_examples = 20_000;
    let app_args: Vec<_> = args().collect();
    if let Some(index) = app_args.iter().position(|e| e == "--max-examples") {
        max_examples = min(all_examples.len(), app_args[index + 1].parse::<usize>().unwrap());
    }

    let use_par_iter = app_args.contains(&("--parallel".to_string()));

    let mut rng = thread_rng();
    all_examples.shuffle(&mut rng);

    println!("Running {} bench on {} examples",
             if use_par_iter { "parallel"} else { "single-threaded"},
             max_examples);

    let now = Instant::now();
    match use_par_iter {
        true => {
            all_examples[0..max_examples]
                .par_iter().for_each(|example| {
                if let Some(_) = lingua_detector.detect_language_of(example) {}
            });
        }
        false => {
            all_examples[0..max_examples]
                .iter().for_each(|example| {
                if let Some(_) = lingua_detector.detect_language_of(example) {}
            });
        }
    }
    let elapsed_millis = now.elapsed().as_millis();
    let detection_per_sec = max_examples as f64 / (elapsed_millis as f64 / 1000.0);
    println!("Finished bench");
    println!("Elapsed = {} ms, Detections per second = {}", elapsed_millis, detection_per_sec);
